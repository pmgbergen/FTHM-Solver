...
from pp_utils import MyPetscSolver, DymanicTimeStepping, StatisticsSavingMixin
...
class Fpm3(
    ...
    def before_nonlinear_loop(self) -> None:
        ...
        displacement = self.displacement(self.mdg.subdomains(dim=self.nd))
        displacement = displacement.value(self.equation_system)
        print()
        print(f'max displacement: {max(abs(displacement)):.1e}')
        pressure = self.pressure(self.mdg.subdomains()).value(self.equation_system)
        print(f"max pressure: {max(abs(pressure)):.1e}")
...
def make_model():
    ...
    time_manager = pp.TimeManager(
        dt_init=dt, dt_min_max=(0.1, 0.5), schedule=[0, 3, 6], constant_dt=False
    )
